    ...
    <div>
      <div class="content">
          ...
          <v-avatar v-else :size="128">
            <img class="profilepic" src="@/assets/defaultpic.png" alt= "profile">
          </v-avatar>
        <p>
        ...
        <p>
          <v-btn v-if="loggedInUser && !this.followed" v-on:click.native="follow" color="primary">Follow</v-btn>
          <v-btn v-if="loggedInUser && this.followed" v-on:click.native="unfollow" color="error">Unfollow</v-btn>
        </p>
        <v-divider></v-divider>
...
<script>
  ...
  export default {
    ...
    mounted() {
        this.followed = this.loggedInUser.follows.some(el => {
